...
class _SavingGoalsScreenState extends State<SavingGoalsScreen>
  ...
  Future<void> _loadGoals() async {
    print("DEBUG: Starting to load goals...");
    try {
      final goals = await _api.getGoals();
      print("DEBUG: Goals loaded: ${goals.length} items");
      if (mounted) {
        setState(() {
          _goals = goals;
          _isLoading = false;
  ...
  void _testGoalsLoading() {
    print("DEBUG: Testing goals loading...");
    _loadGoals();
  }
  ...
  void _testConnection() async {
    ...
    try {
      final response = await _api.testConnection();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          Padding(
            padding: const EdgeInsets.only(right: 15),
            child: IconButton(
              icon: Icon(Icons.refresh, color: AppTheme.primaryColor, size: 24),
              onPressed: _testGoalsLoading,
            ),
          ),
          Padding(
            padding: const EdgeInsets.only(right: 15),
      ...
      floatingActionButton: AnimatedBuilder(
        ...
        child: FloatingActionButton.extended(
          heroTag: 'fab_savings_goals_unique',
          onPressed: _showCreateGoalDialog,
