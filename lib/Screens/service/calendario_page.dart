...
class _CalendarioPageState extends State<CalendarioPage> {
  ...
  bool _isLoadingPage =
      true; // Renombrado para diferenciarlo de la carga de acciones
  ...
  //       TextEditingController(text: isEditing ? occurrence.title : '');
  //   final amountController = TextEditingController(
  //       text: isEditing ? occurrence.amount.toString() : '');
  ...
  //                 Text(
  //                     isEditing ? 'Editar Recordatorio' : 'Añadir Recordatorio',
  ...
  //                   Text(
  //                       'para ${DateFormat.yMMMMd('es').format(_selectedDay!)}',
  //                       style: Theme.of(context)
  //                           .textTheme
  //                           .titleMedium
  ...
  //                                       0.0,
  //                               'type': typeValue,
  //                               'start_date': DateFormat('y-MM-dd').format(
  //                                   isEditing
  //                                       ? DateTime.parse(occurrence.date)
  //                                       : dayToRegister),
  ...
  //                             bool success = false;
  //                             if (isEditing) {
  //                               success = await _controller
  //                                   .updateScheduledTransaction(
  //                                       occurrence.id, data, context);
  //                             } else {
  //                               final newOccurrence = await _controller
  ...
  //                                     _focusedDay); // Refresca los datos en la página principal
  //                               } else {
  ...
  //                           onPressed: () async {
  //                             final success =
  //                                 await _controller.deleteScheduledTransaction(
  //                                     occurrence.id, context);
  //                             if (success && mounted) {
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      Textwidget(text: '¡Estás al día! Toca un día futuro para añadir un nuevo recordatorio.', maxLines: 2, fontWeight: FontWeight.normal, size: 16, color: Colors.grey[500]!, textAlign: TextAlign.center,)
    ]));
  }
                                                          ...
                                                          : () async {
                                                              setState(() =>
                                                                  _isLoadingPage =
                                                                      true);
                                                              final success =
                                                                  await _controller.updatePaidStatus(
                                                                      transactionId:
                                                                          gasto
                                                                              .id,
                                                                      date: gasto
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          ...
          if (!normalizedDay.isBefore(_today)) {
            showAddOrEditGastoBottomSheet(context);
          } else {
